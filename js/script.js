...
const preencheDadosDasSalgados = (salgadoItem, item2, index2) => {
    // aula 05
    // setar um atributo para identificar qual elemento foi clicado
	salgadoItem.setAttribute('data-key', index2)
    salgadoItem.querySelector('.salgado-item--img img').src = item2.img
    salgadoItem.querySelector('.pizza-item--price').innerHTML = formatoReal(item2.price[2])
    salgadoItem.querySelector('.pizza-item--name').innerHTML = item2.name
    salgadoItem.querySelector('.pizza-item--desc').innerHTML = item2.description
}
...
const preencherTamanhos = (key) => {
    ...
    selecionaTodos('.pizzaInfo--size').forEach((size, sizeIndex) => {
        ...
        size.querySelector('span').innerHTML = bolosJson[key].sizes[sizeIndex]
    })
}
...
const escolherTamanhoPreco = (key) => {
    ...
    selecionaTodos('.pizzaInfo--size').forEach((size, sizeIndex) => {
        size.addEventListener('click', (e) => {
            ...
            seleciona('.pizzaInfo--actualPrice').innerHTML = formatoReal(bolosJson[key].price[sizeIndex])
        })
    })
}
...
const adicionarNoCarrinho = () => {
    seleciona('.pizzaInfo--addButton').addEventListener('click', () => {
	    ...
	    let identificador = bolosJson[modalKey].id+'t'+size
...
const adicionarNoCarrinho2 = () => {
    seleciona('.pizzaInfo--addButton').addEventListener('click', () => {
        ...
        // crie um identificador que junte id e tamanho
	    // concatene as duas informacoes separadas por um símbolo, vc escolhe
	    let identificador = salgadosJson[modalKey].id+'t'+size

        // antes de adicionar verifique se ja tem aquele codigo e tamanho
        // para adicionarmos a quantidade
        let key = cart.findIndex( (item2) => item2.identificador == identificador )
        console.log(key)

        if(key > -1) {
            // se encontrar aumente a quantidade
            cart[key].qt += quantSalgados
        } else {
            // adicionar objeto pizza no carrinho
            let salgado = {
                identificador,
                id: salgadosJson[modalKey].id,
                size, // size: size
                qt: quantSalgados,
                price: parseFloat(price) // price: price
            }
            cart.push(salgado)
            console.log(salgado)
            console.log('Sub total R$ ' + (salgado.qt * salgado.price).toFixed(2))
        }
...
const adicionarNoCarrinho3 = () => {
    seleciona('.pizzaInfo--addButton').addEventListener('click', () => {
        ...
        // crie um identificador que junte id e tamanho
	    // concatene as duas informacoes separadas por um símbolo, vc escolhe
	    let identificador = bebidasJson[modalKey].id+'t'+size
...
const atualizarCarrinho = () => {
	...
	if(cart.length > 0) {
		...
		for(let i in cart) {
			...
			let pizzaItem = bolosJson.find( (item) => item.id == cart[i].id )
...
const atualizarCarrinho2 = () => {
	...
	if(cart.length > 0) {
		...
		for(let i in cart) {
			// use o find para pegar o item por id
			let salgadoItem = salgadosJson.find( (item2) => item2.id == cart[i].id )
			...
			cartItem.querySelector('.cart--item-nome').innerHTML = salgadoName
			cartItem.querySelector('.cart--item--qt').innerHTML = cart[i].qt
...
bolosJson.map((item, index ) => {
    //console.log(item)
    let pizzaItem = document.querySelector('.models .pizza-item').cloneNode(true)
    //console.log(pizzaItem)
    //document.querySelector('.pizza-area').append(pizzaItem)
    seleciona('.bolo-area').append(pizzaItem)
...
salgadosJson.map((item2, index2 ) => {
    //console.log(item)
    let salgadoItem = document.querySelector('.models .pizza-item').cloneNode(true)
    //console.log(pizzaItem)
    //document.querySelector('.pizza-area2').append(pizzaItem)
    seleciona('.salgado-area').append(salgadoItem)

    // preencher os dados de cada pizza
    preencheDadosDasPizzas(salgadoItem, item2, index2)
    
    // pizza clicada
    salgadoItem.querySelector('.pizza-item a').addEventListener('click', (e) => {
        e.preventDefault()
        console.log('Clicou na pizza')
...
bebidasJson.map((item, index ) => {
    ...
    //console.log(pizzaItem)
    //document.querySelector('.pizza-area2').append(pizzaItem)
    seleciona('.bebida-area').append(pizzaItem)
